...
function initGame() {
  ...
  const tutorialTooltipClose = document.getElementById("tutorialTooltipClose");
  const tutorialTooltipOk = document.getElementById("tutorialTooltipOk");
  ...
  // Tooltip dialog buttons
  if (tutorialTooltipClose) {
    tutorialTooltipClose.addEventListener("click", () => {
      hideTutorialTooltip();
    });
  }

  if (tutorialTooltipOk) {
    tutorialTooltipOk.addEventListener("click", () => {
      hideTutorialTooltip();
    });
  }

  // Close dialog when clicking outside
  const tutorialDialog = document.getElementById("tutorialTooltipDialog");
  if (tutorialDialog) {
  ...
  if (tutorialNextBtn) {
    tutorialNextBtn.addEventListener("click", () => {
      if (window.tutorialStep < maxTutorialSteps) {
        window.tutorialStep++;
        updateTutorialStep();
      } else {
        hideTutorial();
...
const maxTutorialSteps = 4;
let tutorialRuneGuess = [null, null, null, null];
const tutorialSolution = ["O", "P", "E", "N"];
...
function showTutorial() {
  ...
  if (tutorialScreen) {
    tutorialScreen.classList.add("screen--visible");
  }
  if (splashScreen) {
    splashScreen.classList.remove("screen--visible");
  }
...
function hideTutorial() {
  const tutorialScreen = document.getElementById("tutorial-screen");
  if (tutorialScreen) {
    tutorialScreen.classList.remove("screen--visible");
  }
}
...
function updateTutorialStep() {
  ...
  steps.forEach((step, index) => {
    if (index + 1 === window.tutorialStep) {
  ...
  if (prevBtn) {
    prevBtn.disabled = window.tutorialStep === 1;
  }
  ...
  // Show tooltip dialog based on step
  showTutorialTooltip();
}

function showTutorialTooltip() {
  const dialog = document.getElementById("tutorialTooltipDialog");
  const title = document.getElementById("tutorialTooltipTitle");
  const text = document.getElementById("tutorialTooltipText");
  ...
  const messages = {
    1: {
      ...
      text: "Each level gives you a clue. Here it says 'A barrier you can open' - think about what word that might be! The clue will help you figure out which runes to select."
    },
    2: {
      title: "Step 2: Select Runes",
      text: "Tap runes from the keyboard below to fill the slots. The letter will appear above each slot as you select it. Try tapping different runes to see how they work!"
    },
    3: {
      title: "Step 3: Fill All Slots",
      text: "Once all slots are filled, the 'Reveal' button will be enabled. Tap it to check your answer! If you're correct, you'll see the word revealed."
    },
    4: {
      title: "Step 4: Use the Codex",
      text: "The Codex shows all rune families (F1-F6). Use it as a reference to see all available runes, but remember - it doesn't show which letter each rune represents! You'll need to figure that out through the clues."
    }
  };
  
  const message = messages[window.tutorialStep];
  if (message) {
...
function setupTutorialPuzzle() {
  // Setup tutorial slots
  const slotsContainer = document.getElementById("tutorialRuneSlots");
  if (!slotsContainer) return;
  
  slotsContainer.innerHTML = "";
  tutorialRuneGuess = [null, null, null, null];
  
  for (let i = 0; i < 4; i++) {
    const wrapper = document.createElement("div");
    wrapper.className = "rune-slot-wrapper";
  ...
  // Setup tutorial keyboard
  renderTutorialKeyboard();
  
  // Setup tutorial family tabs
  renderTutorialFamilyTabs();
}

function renderTutorialFamilyTabs() {
  const tabsContainer = document.getElementById("tutorialFamilyTabs");
  if (!tabsContainer) return;
  
  tabsContainer.innerHTML = "";
  
  RUNE_FAMILIES.forEach(family => {
    const btn = document.createElement("button");
    btn.className = "rune-family-tab";
    if (family.id === "F1") {
      btn.classList.add("rune-family-tab--active");
    }
    btn.textContent = family.label;
    btn.dataset.family = family.id;
    
    btn.addEventListener("click", () => {
      document.querySelectorAll("#tutorialFamilyTabs .rune-family-tab").forEach(t => {
        t.classList.remove("rune-family-tab--active");
      });
      btn.classList.add("rune-family-tab--active");
...
function renderTutorialKeyboard() {
  const keyboardContainer = document.getElementById("tutorialKeyboard");
  if (!keyboardContainer) return;
  ...
  rows.forEach((row) => {
    ...
    row.forEach((letter) => {
      ...
      key.addEventListener("click", () => {
        handleTutorialRuneClick(letter);
      });
...
function handleTutorialRuneClick(letter) {
  const firstEmpty = tutorialRuneGuess.findIndex(r => r === null);
  if (firstEmpty === -1) return;
  
  tutorialRuneGuess[firstEmpty] = { letter, family: activeFamilyId };
  updateTutorialSlots();
  
  const revealBtn = document.getElementById("tutorialRevealBtn");
  if (revealBtn && tutorialRuneGuess.every(r => r !== null)) {
    revealBtn.disabled = false;
  }
}

function updateTutorialSlots() {
  const wrappers = document.querySelectorAll("#tutorialRuneSlots .rune-slot-wrapper");
  wrappers.forEach((wrapper, index) => {
    const slot = wrapper.querySelector(".rune-slot");
    const letterLabel = wrapper.querySelector(".rune-slot-letter");
    const rune = tutorialRuneGuess[index];
...
function setupTutorialRunes() {
  const revealBtn = document.getElementById("tutorialRevealBtn");
  const clearBtn = document.getElementById("tutorialClearBtn");
  
  if (revealBtn) {
    revealBtn.addEventListener("click", () => {
      const isCorrect = tutorialRuneGuess.every((rune, i) => {
        return rune && rune.letter === tutorialSolution[i];
      });
      ...
      if (isCorrect) {
        if (dialog && title && text) {
          ...
          text.textContent = "Great job! The word is OPEN. You've successfully solved your first puzzle!";
          dialog.classList.add("visible");
        }
      } else {
        if (dialog && title && text) {
          title.textContent = "Not Quite Right";
          text.textContent = "That's not the correct answer. Try again! Remember to match the runes to the letters in the word.";
          dialog.classList.add("visible");
        }
      }
    });
